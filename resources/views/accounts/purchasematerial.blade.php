    ...
    <main class="main-content position-relative max-height-vh-100 h-100 mt-1 border-radius-lg">
                                ...
                                <div class="table-responsive">
                                    <table class="table table-bordered" id="itemsTable">
                                        <thead>
                                            <tr>
                                                <th width="5%">No.</th>
                                                <th>R/NR</th> <!-- [MODIFIKASI] Kolom R/NR untuk Routine/Non-Routine -->
                                                <th>Business Partner</th>
                                                <!-- [MODIFIKASI] Kolom Business Partner input dinamis -->
                                                <th>Line Of Business</th> <!-- [MODIFIKASI] Kolom Line Of Business -->
                                                <th>Item</th>
                                                <th>Description</th>
                                        ...
                                        <tbody>
                                            @if (session('temp_data'))
                                                @foreach (session('temp_data') as $index => $data)
                                                    <tr>
                                                        <td>{{ $index + 1 }}</td>
                                                        <td>{{ $budget_codes[$data['bdc_id']] ?? ($data['bdc_id'] === 'R' ? 'Routine' : ($data['bdc_id'] === 'NR' ? 'Non-Routine' : '-')) }}
                                                        </td>
                                                        <!-- [MODIFIKASI] Gunakan budget_codes untuk menampilkan nama budget dan fallback ke Routine/Non-Routine -->
                                                        <td>{{ $data['business_partner'] ?? '-' }}</td>
                                                        <!-- [MODIFIKASI] Kolom Business Partner dari input dinamis -->
                                                        <td>{{ $line_business[$data['lob_id']] ?? ($data['lob_id'] ?? '-') }}
                                                        </td> <!-- [MODIFIKASI] Kolom Line Of Business -->
                                                        <td>{{ $data['itm_id'] ?? '-' }}</td>
                                                        ...
                                                        <td>{{ $workcenters[$data['wct_id']] ?? ($data['wct_id'] ?? '-') }}
                                                        </td>
                                                        <td>{{ $departments[$data['dpt_id']] ?? ($data['dpt_id'] ?? '-') }}
                                                        </td>
                                                        <td>{{ $data['month'] ?? '-' }}</td>
                                            ...
                                            @else
                                                <tr>
                                                    <td colspan="12" class="text-center text-muted">No items added yet
                                                    </td> <!-- [MODIFIKASI] Colspan 12 sesuai jumlah kolom -->
                                                </tr>
                                            @endif
                                        </tbody>
                                    </table>
                                ...
                                <div class="d-flex justify-content-between mt-4">
                                    ...
                                    <div class="d-flex">
                                        <button type="button" id="cancelBtn" class="btn btn-secondary me-2"
                                            data-bs-toggle="modal" data-bs-target="#cancelConfirmModal">Cancel</button>
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <!-- [MODIFIKASI] Ganti field rnr menjadi bdc_id dan gunakan budget_codes -->
                                        <select name="bdc_id" id="bdc_id" class="form-control" required>
                                    ...
                                    <div class="mb-3">
                                        <label class="form-label">Business Partner</label><span
                                            class="text-danger">*</span>
                                        <!-- [MODIFIKASI] Input dinamis untuk Business Partner -->
                                        <input type="text" name="business_partner" id="business_partner"
                                    ...
                                    <div class="mb-3">
                                        <label class="form-label">Line Of Business</label><span
                                            class="text-danger">*</span>
                                        <!-- [MODIFIKASI] Input Line Of Business dari line_business -->
                                        <select name="lob_id" id="lob_id" class="form-control" required>
                                            <option value="">-- Select Line Of Business --</option>
                                            @foreach ($line_business as $lob_id => $line)
        ...
        <script>
            $(document).ready(function() {
                ...
                .select2({ // [MODIFIKASI] Tambahkan bdc_id ke inisialisasi Select2

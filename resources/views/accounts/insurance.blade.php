...
<body class="g-sidenav-show bg-gray-100">
    ...
    <main class="main-content position-relative max-height-vh-100 h-100 mt-1 border-radius-lg">
        ...
        <div class="container-fluid">
            @if ($acc_id == 'FOHINSPREM' || $acc_id == 'SGAINSURANCE')
                <div class="row">
                    <!-- Purpose Card -->
                                            ...
                                            <tbody>
                                                @if (session('temp_data'))
                                                    @foreach (session('temp_data') as $index => $data)
                                                        <tr>
                                                            ...
                                                            <td>{{ $insurance_prems[$data['ins_id']] ?? $data['ins_id'] }}
                                                            </td>
                                                            <!-- [MODIFIKASI] Kolom Qty dihapus -->
                                                            <td class="text-end">
                                                            ...
                                                            <td class="text-end">
                                                                <!-- [MODIFIKASI] Amount langsung berdasarkan price -->
                                                                {{ isset($data['price']) ? 'IDR ' . number_format($data['price'], 2) : '-' }}
                                                            </td>
                                                ...
                                                @else
                                                    <tr>
                                                        ...
                                                        <td colspan="7" class="text-center text-muted">No items added
                                                            yet</td>
                                                    </tr>
                                                @endif
                                            </tbody>
                                    ...
                                    <div class="modal fade" id="cancelConfirmModal" tabindex="-1"
                                        ...
                                        <div class="modal-dialog modal-dialog-centered">
                                            <div class="modal-content text-center p-4" style="border-radius: 15px;">
                                                ...
                                                <h5 class="aldo-title fw-bold mb-2" id="cancelConfirmModalLabel">Cancel
                                                    Submission</h5>
                        ...
                        <div class="modal-body">
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    ...
                                    <div class="mb-3">
                                        <label class="form-label">Insurance Company<span
                                                class="text-danger">*</span></label>
                                        <select name="ins_id" class="form-control" required id="ins_id">
                                            <option value="">-- Insurance Company --</option>
                                            @foreach ($insurance_prems as $insID => $insurance_prem)
                                                <option value="{{ $insID }}" @selected(old('ins_id') == $insID)>
                                                    {{ $insurance_prem }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
        ...
        <script>
            $(document).ready(function() {
                ...
                $('#cur_id').on('change', function() {
                    ...
                    $('#price').trigger('input'); // [MODIFIKASI] Hapus trigger quantity
                });

                $('#price, #cur_id').on('input', function() { // [MODIFIKASI] Hapus quantity dari event listener
                    const price = parseFloat($('#price').val()) || 0;
                    const cur_id = $('#cur_id').val();
                    const currency = currencies.find(c => c.cur_id === cur_id);
                    const nominal = currency ? parseFloat(currency.nominal) : 1;
                    const amount = price * nominal; // [MODIFIKASI] Amount langsung berdasarkan price
                ...
                $('#mainForm').on('submit', function(e) {
                    // Check if the items table has any data rows

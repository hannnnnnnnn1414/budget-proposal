                                    ...
                                    @php
                                        ...
                                        $groupedItems = $submissions
                                            ->groupBy(function ($submission) {
                                                return ($submission->ledger_account ?? '-') .
                                                    '-' .
                                                    ($submission->ledger_account_description ?? '-') .
                                                    '-' .
                                                    ($submission->workcenter ?? '-');
                                            })
                                            ->map(function ($group) use ($monthMap, $monthLabels, $allMonths) {
                                                $first = $group->first();
                                                $monthlyData = [];
                                                $totalAmount = 0;
                                                ...
                                                // Isi data dari submissions
                                                foreach ($group as $submission) {
                                                    $month = isset($monthMap[$submission->month])
                                                        ? $monthMap[$submission->month]
                                                        : null;
                                                    if ($month && array_key_exists($month, $monthlyData)) {
                                                        $monthlyData[$month] = $submission->price;
                                                        $totalAmount += $submission->price;
                                                    }
                                                }

                                                return [
                                                    'ledger_account' => $first->ledger_account ?? '-',
                                                    'ledger_account_description' =>
                                                        $first->ledger_account_description ?? '-',
                                                    'bdc_id' => $first->bdc_id ?? '-',
                                                    'lob_id' => $first->lob_id ?? '-',
                                                    ...
                                                    'months' => $monthlyData, // Sekarang $monthlyData sudah terdefinisi
                                        ...
                                        $months = $allMonths; // Gunakan $allMonths yang sudah didefinisikan
                                        $grandTotal = $groupedItems->sum('total_amount'); // Perbaikan: gunakan 'total_amount'
                                    @endphp
                                    ...
                                    <div class="table-responsive" style="max-height: 70vh; overflow-y: auto;">
                                        <table class="table table-bordered"
                                            ...
                                            <tbody>
                                                    ...
                                                    <tr class="hover:bg-gray-50">
                                                        ...
                                                        @foreach ($months as $month)
                                                            <td class="border p-2 text-center">
                                                                @if ($item['months'][$month] > 0)
                                                                    @php
                                                                        // Cari data monthly yang sesuai
                                                                        $monthlyData = $submissions->first(function (
                                                                            $submission,
                                                                        ) use ($month, $item, $monthMap) {
                                                                            return $monthMap[$submission->month] ===
                                                                                $month &&
                                                                                $submission->ledger_account ===
                                                                                    $item['ledger_account'] &&
                                                                                $submission->ledger_account_description ===
                                                                                    $item['ledger_account_description'];
                                                                        });
                                                                    @endphp
                                                                    @if ($item['status'] == 2 || $item['status'] == 9)
                                                                        <a href="#" class="editable-month"
                                                                            data-sub-id="{{ $item['sub_id'] }}"
                                                                            data-id="{{ $monthlyData->id ?? $item['id'] }}"
                                                                            data-month="{{ $month }}"
                                                                            data-price="{{ $monthlyData->price ?? $item['months'][$month] }}"
                                                                            data-ledger-account="{{ $item['ledger_account'] }}"
                                                                            data-ledger-account-description="{{ $item['ledger_account_description'] }}"
                                                ...
                                                @empty
                                                    <tr>
                                                        <td colspan="{{ count($months) + 7 }}"
                                                            class="border p-2 text-center">No Submissions found!
                                                        </td>
                            ...
                            @php
                                ...
                                $groupedItems = $submissions
                                    ...
                                    ->map(function ($group) use ($monthMap, $monthLabels, $allMonths) {
                                        ...
                                        return [
                                            'ledger_account' => $first->ledger_account ?? '-',
                                            'ledger_account_description' => $first->ledger_account_description ?? '-',
                                            'bdc_id' => $first->bdc_id ?? '-',
                                            'lob_id' => $first->lob_id ?? '-',
                                                        ...
                                                        @if ($item['months'][$month] > 0)
                                                            @php
                                                                ...
                                                                $monthlyData = $submissions->first(function (
                                                                    ...
                                                                    return $monthMap[$submission->month] === $month &&
                                                                        $submission->ledger_account ===
                                                                            $item['ledger_account'] &&
                                                                        $submission->ledger_account_description ===
                                                                            $item['ledger_account_description'];
                ...
                @php
                    ...
                    $groupedItems = $submissions
                        ...
                        ->map(function ($group) use ($monthMap, $monthLabels, $allMonths) {
                            ...
                            // Isi data dari submissions
                            foreach ($group as $submission) {
                                $month = isset($monthMap[$submission->month]) ? $monthMap[$submission->month] : null;
                                if ($month && array_key_exists($month, $monthlyData)) {
                                    $monthlyData[$month] = $submission->price;
                                    $totalAmount += $submission->price;
                                }
                ...
                <div class="table-responsive" style="max-height: 70vh; overflow-y: auto;">
                    <table class="table table-bordered"
                        ...
                        <tbody>
                                    ...
                                    @foreach ($months as $month)
                                        <td class="border p-2 text-center">
                                            @if ($item['months'][$month] > 0)
                                                @php
                                                    // Cari data monthly yang sesuai
                                                    $monthlyData = $submissions->first(function ($submission) use (
                                                        $month,
                                                        $item,
                                                        $monthMap,
                                                    ) {
                                                        return $monthMap[$submission->month] === $month &&
                                                            $submission->ledger_account === $item['ledger_account'] &&
                                                            $submission->ledger_account_description ===
                                                                $item['ledger_account_description'];
                                                    });
                                                @endphp
                            ...
                            @empty
                                <tr>
                                    <td colspan="{{ count($months) + 7 }}" class="border p-2 text-center">No
                                        Submissions found!
                                    </td>
        ...
        <div class="d-flex justify-content-between mt-4">
            ...
            <div class="d-flex">
                @if ($submission->status == 5)
                    ...
                    <form action="{{ route('submissions.disapprove', $submission->sub_id) }}" method="POST"
                        class="disapprove-form">
                        @csrf
                        <button type="submit" class="btn btn-secondary">
                            <i class="fa-solid fa-xmark me-2"></i> DISAPPROVED
                        </button>
                        ...
                        <form id="editMonthForm" method="POST" action="">
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="text" id="edit_month_ledger_account" class="form-control"
                                        readonly>
                                </div>
                            </div>
                            ...
                            <div class="row mb-3">
                                <div class="col-md-6">
                                    <label class="form-label">Ledger Account Description</label>
                                    <textarea id="edit_month_ledger_account_description" class="form-control" readonly></textarea>
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label">Budget Code <span class="text-danger">*</span></label>
                                    <select name="bdc_id" id="edit_month_bdc_id" class="form-control select"
                                        required>
                                        @foreach (\App\Models\BudgetCode::orderBy('budget_name', 'asc')->get() as $budgetCode)
                                            <option value="{{ $budgetCode->bdc_id }}">
                                                {{ $budgetCode->budget_name }}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-md-6">
                                    <label for="edit_month_cur_id" class="form-label">Mata Uang <span
                                            class="text-danger">*</span></label>
                            ...
                            <div class="row mb-3">
                                <div class="col-md-6">
                                    <label for="edit_month_price" class="form-label">Harga <span
                                            class="text-danger">*</span></label>
                        ...
                        <form id="addItemForm" method="POST"
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select name="lob_id" id="lob_id" class="form-control select" required>
                                            @foreach ($line_businesses as $lob_id => $line)
                                                <option value="{{ $lob_id }}" @selected(old('lob_id') == $lob_id)>
                                                    {{ $line }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="mb-3">
                                        <label class="form-label">Ledger Account <span
                                                class="text-danger">*</span></label>
                                        <input type="text" name="ledger_account" id="ledger_account"
                                            class="form-control" placeholder="Masukkan ledger account" required
                                            value="{{ old('ledger_account') }}">
                                    </div>
                                    <div class="mb-3">
                                        <label class="form-label">Ledger Account Description <span
                                                class="text-danger">*</span></label>
                                        <textarea class="form-control" name="ledger_account_description" id="ledger_account_description"
                                            placeholder="Masukkan deskripsi ledger account" required>{{ old('ledger_account_description') }}</textarea>
                                    </div>
                                    <div class="mb-3">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <div class="mb-3">
                                        <label for="amountDisplay" class="form-label">Jumlah (IDR)</label>
        ...
        <script>
            $(document).ready(function() {
                ...
                $(document).on('click', '.editable-month', function(e) {
                    ...
                    const ledgerAccount = $(this).data('ledger-account');
                    const ledgerAccountDescription = $(this).data('ledger-account-description');
                    ...
                    $('#edit_month_ledger_account').val(ledgerAccount);
                    $('#edit_month_ledger_account_description').val(ledgerAccountDescription);
            ...
            $(document).on('submit', '#addItemForm', function(e) {
                ...
                const requiredFields = [
                    '#bdc_id', '#lob_id', '#ledger_account', '#ledger_account_description',
                    '#price', '#wct_id', '#month', '#cur_id'
                ];
                let missingFields = [];

                requiredFields.forEach(field => {
                    if (!form.find(field).val()) {
                        const fieldName = field.replace('#', '').replace(/_/g, ' ');
                ...
                if (missingFields.length > 0) {
                    Swal.fire({
                        icon: 'error',
                        title: 'Error!',
                        text: 'Harap isi semua field wajib: ' + missingFields.join(', '),
                        confirmButtonColor: '#d33'
                    });
                    return;
                }

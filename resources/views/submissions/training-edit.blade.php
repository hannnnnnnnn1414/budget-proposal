...
<body class="g-sidenav-show bg-gray-100">
    ...
    <div class="modal-content">
        ...
        <form id="updateSubmissionForm" action="{{ route('submissions.update', ['sub_id' => $submission->sub_id, 'id' => $submission->id]) }}" method="POST">
            @csrf
            @method('PUT')
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" name="participant"
                                value="{{ old('participant', $submission->participant) }}" placeholder="Participant">
                        </div>

                        <div class="mb-3">
                            <label class="form-label">Jenis Training</label><span class="text-danger">*</span>
                            <input type="text" class="form-control" required name="jenis_training"
                                value="{{ old('jenis_training', $submission->jenis_training) }}" placeholder="Jenis Training" required>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control" required id="quantity" name="quantity"
                                value="{{ old('quantity', $submission->quantity) }}" placeholder="Quantity" required>
                        </div>
                        <div class="mb-3">
                            <div class="row g-2">
                                <!-- Currency (kiri) -->
    ...
    $(document).ready(function() {
        // Function to calculate and update amount display
        // function calculateAmount() {
        //     // Scope selectors to #editModal
        //     const $modal = $('#editModal');
        //     const $quantityInput = $modal.find('input#quantity');
        //     const $priceInput = $modal.find('input#price');
        //     const $amountDisplay = $modal.find('input#amountDisplay');
        //     const $amountHidden = $modal.find('input#amount');

        //     // Check if elements exist
        //     if (!$quantityInput.length || !$priceInput.length || !$amountDisplay.length || !$amountHidden.length) {
        //         console.error('Missing input elements in #editModal:', {
        //             quantity: $quantityInput.length,
        //             price: $priceInput.length,
        //             amountDisplay: $amountDisplay.length,
        //             amount: $amountHidden.length
        //         });
        //         return;
        //     }
        ...
        //     // Debugging: Log raw and parsed values
        //     console.log('Raw Quantity:', $quantityInput.val(), 'Parsed Quantity:', quantity);
        //     console.log('Raw Price:', $priceInput.val(), 'Parsed Price:', price);
        //     console.log('Calculated Amount:', amount);

        //     // Update display field with IDR format
        //     $amountDisplay.val('IDR ' + amount.toLocaleString('id-ID', {
        //         minimumFractionDigits: 2,
        //         maximumFractionDigits: 2
        //     }));

        //     // Update hidden input for amount
        //     $amountHidden.val(amount.toFixed(2));
        // }

        // // Initialize modal content (Select2 and event listeners)
        // function initializeModal() {
        //     const $modal = $('#editModal');

        //     // Initialize Select2 for select elements
        //     $modal.find('.form-control.select').select2({
        //         width: '100%',
        //         dropdownParent: $modal
        //     });

        //     // Unbind previous event listeners to prevent duplicates
        //     $modal.find('input#quantity, input#price').off('input.calculateAmount');

        //     // Bind event listeners for quantity and price inputs
        //     $modal.find('input#quantity, input#price').on('input.calculateAmount', function() {
        //         console.log('Input event triggered for:', $(this).attr('id'), 'Value:', $(this).val());
        //         calculateAmount();
        //     });
        ...
        $('#editModal').on('shown.bs.modal', function() {
            ...
            console.log('Modal HTML:', $('#editModal').html());
            initializeModal();
        });

        // Initial call in case modal is already loaded
        initializeModal();
    });
</script>
</body>

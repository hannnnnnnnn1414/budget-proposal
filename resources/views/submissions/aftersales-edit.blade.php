            ...
            <div class="modal-body">
                ...
                <div class="row">
                    <div class="col-md-6">
                        ...
                        <div class="mb-3">
                            <label class="form-label">Customer</label><span class="text-danger">*</span>
                            <input type="text" class="form-control" required name="customer"
                                value="{{ old('customer', $submission->customer) }}" placeholder="Customer">
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="mb-3">
            ...
            function initializeModal() {
                ...
                $modal.find('#input_type').off('change').on('change', function() {
                    ...
                    if ($(this).val() === 'select') {
                        ...
                        // Clear itm_id and keep description
                        // $itmId.val(''); // Set to empty instead of originalItmId
                        $manualItem.val('');
                        ...
                        // Restore original manual_item and clear itm_id
                        $manualItem.val(originalManualItem);
                        // $itmId.val('');
                        // For manual, restore original description or set to manual_item
                        $description.val(originalDescription || originalManualItem);
                        $itmId.val('').trigger('change'); // Clear Select2
                    }
                });

                // Fetch item description on itm_id keypress (Enter)
                // $modal.find('#itm_id').off('keypress').on('keypress', function(e) {
                //     if (e.which === 13) { // Enter key
                //         e.preventDefault();
                //         const itm_id = $(this).val().trim();
                //         const $description = $modal.find('#description');
                //         if (itm_id && $modal.find('#input_type').val() === 'select') {
                //             fetchItemDescription(itm_id, $description);
                //         } else if (!itm_id) {
                //             $description.val('');
                //             Swal.fire({
                //                 icon: 'error',
                //                 title: 'Error',
                //                 text: 'Item GID cannot be empty',
                ...
                //                 icon: 'error',
                //                 title: 'Error',
                //                 text: 'Please ensure Input Type is set to Item GID',
                //                 confirmButtonColor: '#d33'
                //             });
                //         }
                //     }
                ...
                // // Function to fetch item description via AJAX
                // function fetchItemDescription(itm_id, $description) {
                //     $.ajax({
                ...
                //                     text: 'Item GID "' + itm_id + '" not found in database',
                //                     confirmButtonColor: '#d33'
                //                 });
                //                 $('#itm_id').val('').focus();
                //             }
                //         },
                //         error: function(xhr) {
                //             let errorMessage = 'Item GID not found';
                //             if (xhr.status === 404) {
                //                 // errorMessage = 'Item GID not found';
                //             } else if (xhr.status === 403) {
                //                 errorMessage = 'CSRF token mismatch or unauthorized access';
                //             } else if (xhr.status === 500) {
                //                 errorMessage = 'Server error occurred while fetching item';
                //             }
                //             Swal.fire({
                //                 icon: 'error',
                //                 title: 'Error',
